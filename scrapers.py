...
def money_to_int_or_zero(s: str | None):
    if not s:
        return 0
    s = s.strip().replace("$", "").replace(",", "")
    return int(s) if s.isdigit() else 0

def money_to_float_or_zero(s: str | None):
    if not s:
        return 0.0
    s = s.strip().replace("$", "").replace(",", "")
    try:
        ...
        for i in range(rows.count()):
            ...
            if len(parts) == 7:
                entry = {
                    ...
                    "jockey": parts[2],
                    "trainer": parts[3],
                    "win_pool":  to_int_or_zero(parts[4]),
                    "place_pool":to_int_or_zero(parts[5]),
                    "show_pool": to_int_or_zero(parts[6]),
                }
...
class TwinSpiresScraper(baseScraper): # twinspires needs headless=False to work properly
    ...
    def scrape_race(self, page: Page, url: str | None = None) -> dict:
        ...
        race_num = to_int_or_zero(url_head.split("/")[-1])
        ...
        while i < len(lines):
            ...
            horse_number = to_int_or_zero(lines[i])
            win_odds = lines[i + 1]
            win_pool = money_to_int_or_zero(lines[i + 2])     # TODO: can replace these with money_to_float
            place_pool = money_to_int_or_zero(lines[i + 3])
            show_pool = money_to_int_or_zero(lines[i + 4])
            i += 5
        ...
        for i in range(total_cards):
            ...
            # horse number
            number  = to_int_or_zero(card.locator(".program-number").first.text_content())
    ...
    def scrape_race_results(self, page: Page, url: str) -> dict:
        ...
        for i in range(cards.count()):
            ...
            entry["horse_number"] = to_int_or_zero(card.locator(".entry_col_num .saddle-cloth").first.text_content())
            entry["place"] = i+1
            entry["win_value"] = money_to_float_or_zero(card.locator(".entry_col_win").first.text_content())
            entry["place_value"] = money_to_float_or_zero(card.locator(".entry_col_place").first.text_content())
            entry["show_value"] = money_to_float_or_zero(card.locator(".entry_col_show").first.text_content())
        ...
        for i in range(cards.count()):
            card = cards.nth(i)
            horse_num = to_int_or_zero(card.locator(".entry_col_num .saddle-cloth").first.text_content())
